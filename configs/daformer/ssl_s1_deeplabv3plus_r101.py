...
#
# DynamicAnchorModule operates on raw encoder features (2048-d).
# A learned Linear(2048→512) projects prototypes to the decoder's
# fused space before passing through conv_seg for pseudo-label
# correction.
#
# Dimension trace:
#   Encoder: ResNet-101 stages [256, 512, 1024, 2048]
#   DynAnchor input: 2048-d (encoder stage 3)
#   proto_to_decoder: Linear(2048, 512)
#   conv_seg: Conv2d(512, num_classes, 1)
#
# Usage:
#   python tools/train.py configs/daformer/ssl_s1_deeplabv3plus_r101.py
# ---------------------------------------------------------------
...
uda = dict(
    # anchor_after_fusion=False → DynAnchor on encoder features (2048-d)
    # proto_to_decoder = Linear(2048, 512) bridges to conv_seg
    ...
    # DynamicAnchor — feature_dim auto-detected as 2048 (encoder space)
    dynamic_anchor=dict(
...
# --- Optimizer ---
# ResNet-101 is pretrained → conservative base lr.
# DynAnchor prototypes: no weight decay (cluster centres), 10x LR
# (randomly initialised + gradient attenuated through 3 EM iterations).
# proto_to_decoder: 10x LR, no decay (projects cluster centres).
# NOTE: ResNet backbone uses 'backbone' prefix for paramwise matching;
# DeepLabv3+ head uses 'decode_head' which matches 'head'.
...
name = 'ssl_s1_deeplabv3plus_r101'
exp = 'satellite_ssl'
